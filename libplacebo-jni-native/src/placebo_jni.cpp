...
JNIEXPORT jboolean JNICALL Java_com_grill_placebo_PlaceboManager_plRenderAvFrame3
  ...
  if(ui != 0){
      struct ui *ui_instance = reinterpret_cast<struct ui *>(ui);
      render_ui2(ui_instance, globalBtnImage);
  }
...
extern "C"
JNIEXPORT void JNICALL Java_com_grill_placebo_PlaceboManager_nkStoreImageView2
  (JNIEnv *env, jobject obj, jlong imageView) {
  VkImageView pVkImageView = reinterpret_cast<VkImageView>(static_cast<uint64_t>(imageView));
  globalBtnImage = nk_image_ptr(&pVkImageView);
}
